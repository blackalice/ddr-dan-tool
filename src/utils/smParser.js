    ...
    for (; i < lines.length && !concludesANoteTag(lines[i]); ++i) {
      ...
      for (let d = 0; d < cleanedLine.length; ++d) {
        if (cleanedLine[d] === "2") {
          ...
          open[d] = {
            direction: d,
            startOffset: Number(startBeatFraction.n / startBeatFraction.d),
          };
          ...
          open[d].endOffset = Number(endBeatFraction.n / endBeatFraction.d);
          freezes.push(open[d]);
          open[d] = undefined;
        }
      }
